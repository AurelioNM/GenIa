...
class WeatherStorage:
    ...
    async def update_forecast_by_city(
        self, city_name: str, weathers: List[Weather]
    ) -> None:
        try:
            ...
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await cursor.executemany(sql_query, values)
                    await conn.commit()
                    self.logger.info(
        ...
        try:
            ...
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await cursor.execute(sql_query, (city_name,))
                    result = await cursor.fetchall()
                    self.logger.info(f"Result from DB: size={len(result)}")
        ...
        try:
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await cursor.execute(sql, (limit + 1, offset))
    ...
    async def get_paged_cities_weather_v2(
        self,
        page: int,
        size: int,
        correlation,
        ...
        try:
            self.logger.info(
                f"Getting cities weather v2 on DB: limit={limit}, offset={offset}, correlation={correlation}"
            )
            ...
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    self.logger.info(
                        f"Got cities weather v2 on DB: size={len(rows)}, correlation={correlation}"
                    )
